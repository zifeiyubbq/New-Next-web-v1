  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (commitInfo?.commitMessage.description) {
        ...
        const prLink = commitInfo?.commitMessage.summary.replace(prLinkRegex, '[{{input}}](https://github.com/H0llyW00dzZ/ChatGPT-Next-Web/pull/$1/commits)');
        const descriptionWithLinks = commitInfo?.commitMessage.description
          .map((change: string) =>
            change.replace(prLinkRegex, '[{{input}}](https://github.com/H0llyW00dzZ/ChatGPT-Next-Web/pull/$1/commits)')
          )
          .join('\n\n\n');
    ...
    <div className={styles["changelog-page"]}>
      ...
      <div className={styles["changelog-content"]}>
        <div className={styles["markdown-body"]}>
          <iframe
            srcDoc={`<!DOCTYPE html><html><head><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/github-markdown-css/4.0.0/github-markdown.min.css"></head><body class="markdown-body">${mdText}</body></html>`}
            title="Change Log"
